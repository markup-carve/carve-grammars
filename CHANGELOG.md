...
- An author's attribute run survives an HTML round trip. Seventeen node and mark types declared the key/value slot by hand, so Tiptap rendered the map itself and the pairs reached the DOM as `[object Object]`: `89-block-attribute-lines-2` came back without `key=val2`, and a link or image run was lost whole. Every one of them takes the slot from `attributeSlots` now (#506).
- An HTML read-back no longer invents an attribute run out of what a node rendered itself. Nine types collected their own rendered attributes, so a citation came back spelled `{raw="..." integral="false"}`, and the `aria-label`, `aria-labelledby`, `scope`, fence `title` and computed `style` the engines write turned presentation into source. Each type reserves what it renders, and `scope` is reserved on the header cell alone, so an authored `{scope=row}` on a row or a body cell survives (#507, #510). An authored pair whose name the node also renders, such as `{href=...}` on a link or `{title=...}` on an abbreviation, travels through the editor's own HTML in a `data-carve-reserved-kv` attribute instead of overwriting the node's own, so the link keeps its destination and the abbreviation its expansion (#519).
- Prism scopes a fenced code block's language word as `language class-name`. It carried the code body's class before, while TextMate and highlight.js already scoped it (#527).
- The editor's own HTML keeps more of a document through `getHTML()` and `setContent()`. An unresolved link reference stays a reference instead of becoming bare text, a resolved abbreviation no longer comes back with an authored `abbr` run, an aligned table cell no longer gains an authored `align` run, and a raw block stays a raw block instead of turning into a code block (#522, #531, #532). The converter unwraps a link nested inside a link to its text, as the engine renders it (#522).
- A code span inside another mark keeps that mark through the editor's HTML. CarveKit registers its own Code mark with no exclusions, since Carve lets any mark wrap a code span, and turns StarterKit's copy off, so applying code in the editor no longer strips the marks around it. `starterKit: { code: ... }` still configures the mark and `code: false` still removes it (#529).
- Every document the converter produces validates against the kit's schema. 203 of the 1740 corpus documents did not, so parsing the editor's HTML moved or dropped their content: a footnote's second block left the footnote, and a list item opening with a fence was emptied. A list or task item may open with any block or be empty, a footnote takes any blocks, a description and a div may be empty, and a term may stand alone. The converter gives an empty quote an empty paragraph and writes a mark nested in itself once (#537).
...
- A closer may not stand after whitespace in the highlight.js grammar. `bare_closer` requires a non-space in front of the delimiter, so the engine renders `a *b * d` literally and this grammar coloured it - on all five bare delimiters, since emphasis, underline, strong, highlight and strike share one helper. The nine braced constructs are unchanged: the engine does build `a {*b *} d`. The `empty-delimiters` snapshot had recorded the defect as correct and is regenerated (#392).
- The highlight rule's escape guard consumes the escape as a pair instead of counting the backslash run in a lookbehind. The old spelling was bounded at 32 pairs, so all three grammars refused a highlight the engine marks once a run reached 66 backslashes; the TextMate grammar also went inert in TextMate engines that decline a variable-length lookbehind, losing every highlight rather than one. Three shapes stay wrong and are pinned - `x =<== y`, `x =!== y`, `x =a== y` (#390).
- An escaped `=` no longer closes a highlight in any of the three grammars. `x =\= y` renders `x == y` with no mark - the escape makes the character literal - and every grammar closed on it and coloured a run the engine does not. The body may now step over an escaped delimiter as well, so `a =b c\= d= e` reaches the real closer beyond it. The generated sweep grew a backslash, from 1,555 documents to 2,801, which is what made the family visible (#385).
- A bold-italic body may hold the other order's delimiter in the TextMate and Prism grammars. The engine reads `a /*b*c*/ d` and `a */b/c/* d` as combined runs; each body was a run of anything but the closer's own first character, so both were declined and came back plain bold. The canonical body admits any asterisk that does not start the closer; the mirrored one admits a slash only between two word characters, and its opener now refuses a star followed by a star or a space. The measurement behind the asymmetry is at the rule in both grammars (#382).
