...
- A bold-italic body may hold the other order's delimiter in the TextMate and Prism grammars. The engine reads `a /*b*c*/ d` and `a */b/c/* d` as combined runs; each body was a run of anything but the closer's own first character, so both were declined and came back plain bold (#382).
