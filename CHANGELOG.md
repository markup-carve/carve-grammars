...
- TextMate requires the pair to be horizontal-first (carve#1407). `<^`, `~~` and `>v` are alignment runs; the reverse spellings `v>` and `~>` are ordinary cell content, as a lone marker already was.
