...
- An HTML read-back no longer invents an attribute run out of what a node rendered itself. Nine types collected their own rendered attributes, so a citation came back spelled `{raw="..." integral="false"}`, and the `aria-label`, `aria-labelledby`, `scope`, fence `title` and computed `style` the engines write turned presentation into source. Each type reserves what it renders, and `scope` is reserved on the header cell alone, so an authored `{scope=row}` on a row or a body cell survives (#507, #510). A reserved name the author writes as their own pair on the node that generates it is still lost through the round trip, as it was before this release; telling the two apart needs per-value provenance rather than a reserved list.
