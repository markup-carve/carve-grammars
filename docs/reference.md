# Carve Grammars reference

This page documents the package entry points and their behavior. Start with the [package README](../README.md) for a short example.

The TextMate grammar here has a separate lineage from the copies in
[vscode-carve](https://github.com/markup-carve/vscode-carve) and
[intellij-carve](https://github.com/markup-carve/intellij-carve). It has 116
scope names, compared with 151 in vscode-carve, and sometimes uses different
scope families. A scope name in this package does not promise the same name in
an editor repository. Tree-sitter support lives in
[tree-sitter-carve](https://github.com/markup-carve/tree-sitter-carve) and
[zed-carve](https://github.com/markup-carve/zed-carve).

The package follows the structure of
[djot-grammars](https://github.com/php-collective/djot-grammars). Its Tiptap
mapping matches carve-php's `HtmlToCarve` converter, while the highlighting
grammars track Carve's canonical `resources/grammar.ebnf` token set.
